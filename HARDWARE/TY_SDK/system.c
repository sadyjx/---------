...
返回参数 : 无
*****************************************************************************/
static void product_info_cmd_handle()  //产品信息
{
...
返回参数 : 无
*****************************************************************************/
static void zigbee_state_cmd_handle(unsigned char current_state)  //zigbee工作状态
{
	zigbee_net=current_state;		   //  zigbee设备入网状态  0x00：设备为未入网状态  0x01：设备为已入网状态；	 0x02：设备网络状态异常；
...
返回参数 : 无
*****************************************************************************/
static void zigbee_cfg_cmd_handle(void)	  //配置zigbee模块
{
    return;
}
...
功能描述 : ZigBee模块配置信息通知
输入参数 : cmd   
           0x00：将模块软复位，清除堆栈数据，保存网络状态；
           0x01：将模块配置为开始配网状态；
返回参数 : 无
*****************************************************************************/
void zigbee_rst_cmd_handle(unsigned char cmd)//配置zigbee模块   0x00：将模块软复位，清除堆栈数据，保存网络状态；0x01：将模块配置为开始配网状态；
{
  unsigned char buf1[2];
  unsigned char length = 0;
  buf1[0]=cmd;
  length = set_zigbee_uart_buffer(length,(unsigned char *)buf1,1);
...
返回参数 : 无
*****************************************************************************/
void zigbee_rf_test_cmd_handle(unsigned char cmd)//ZigBee模块RF功能测试   cmd 信道 11-26 
{
  unsigned char buf1[2];
  unsigned char length = 0;
  buf1[0]=cmd;
  length = set_zigbee_uart_buffer(length,(unsigned char *)buf1,1);
  zigbee_uart_write_frame(FUNC_TEST_CMD, length); 
...
返回参数 : 无
*****************************************************************************/
static void zigbee_time_get_handle(unsigned char* wdata, unsigned char data_len)
{
    unsigned long time1 =0, time2 = 0;
    time_t localTime, utcTime;
    unsigned char i = 0;
    unsigned char* ptr = wdata;
 ...
 void data_handle(unsigned short offset)
{
    ...
    switch(cmd_type)
    {
        case PRODUCT_INFO_CMD:	//产品信息
			sys_zigbee_use=1;	    //  1 使用ZigBee
            product_info_cmd_handle();
            break;

        case ZIGBEE_STATE_CMD:	//zigbee工作状态
            {
        ...
        case ZIGBEE_CFG_CMD:	//配置zigbee模块
            zigbee_cfg_cmd_handle();
            break;

        case ZIGBEE_DATA_REQ_CMD: //zigbee数据下发命令
            {
        ...
        case DATA_RSP_SYNC_CMD:	 //MCU状态主动上报
        case DATA_RSP_ASYNC_CMD: //MCU状态被动上报
            {
...
#ifdef SERIAL_PROTOCOL_SCENE_ENABLE
        case SCENE_SWITCH_NUM_GET_CMD: 	 //获取场景开关个数（仅用于场景开关）
            zigbee_scene_switch_num_get_handle();
            break;

        case SCENE_SWITCH_ID_REPORT_CMD: //获取场景上报ID（仅用于场景开关）
            {
        ...
        case FUNC_TEST_CMD:	 //zigbee模块功能测试
            {
        ...
        case MCU_OTA_DATA_REQ_CMD:
            ...
            break;
#endif            
        case TIME_GET_CMD:	 //zigbee模块时间同步
